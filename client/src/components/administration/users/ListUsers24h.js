...
const ListUsers24h = () => {
  ...
  const handleLoadMore = async () => {
    ...
    dispatch({
      ...
      page: homeUsers.activeLast24hUsers.page + 1
    });
  ...
  return (
    <div className="container mt-5">
      <div><h5>Total usuarios en las últimas: 24h</h5></div>
        ...
        <table className="table table-bordered table-hover">
          <thead className="thead-dark">
            <tr>
              ...
              <th>user</th>
              <th>Email</th>
              <th>Fecha de Registro</th> {/* Nueva columna para la fecha */}
              <th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            {homeUsers.activeLast24hUsers.map((user, index) => (
              <tr key={user._id}>
                ...
                <td>{user.email}</td>
                <td>{formatDate(user.createdAt)}</td> {/* Mostrar la fecha formateada */}
                <td>
