...
export const updatePost = ({content, images, auth, vente}) => async (dispatch) => {
    let media = []
    const imgNewUrl = images.filter(img => !img.url)
    const imgOldUrl = images.filter(img => img.url)

    if(vente.content === content 
        && imgNewUrl.length === 0
        && imgOldUrl.length === vente.images.length
    ) return;
    ...
    try {
        ...
        const res = await patchDataAPI(`post/${vente._id}`, { 
